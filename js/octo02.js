...
var update = function (clickX, clickY) {
	...
	for (var i = mainIslandCount; i < smallIslandMax + mainIslandCount; i++) {
		...
		if (i < 10) {
			ctx.fillText(i-8, islandArray[i].x - 3, islandArray[i].y + 3);
		} else if (i >= 10 && i < 100) {
			ctx.fillText(i-8, islandArray[i].x - 5, islandArray[i].y + 3);
		} else {
			ctx.fillText(i-8, islandArray[i].x - 8, islandArray[i].y + 3);
		}
	}
	...
	for (i = 1; i < islandArray.length; i++) {
		if (distClick(islandArray[i].x, islandArray[i].y, clickX, clickY) < 15) {
			...
			document.getElementById("island-info").innerHTML = "Island" + String(parseInt(islandArray[i].ID)-8) + ", " + "Controlled by " + islandArray[i].control + ", " + "Population: " + islandArray[i].population + ", " + "Resources: " + islandArray[i].resources;
		}
	}
